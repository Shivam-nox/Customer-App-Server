...
export const getCurrentLocation = (): Promise<LocationCoordinates> => {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      maximumAge: 0, // Always get fresh location for maximum accuracy
    };
...
const fallbackGeocode = async (
  ...
  return {
    ...
    addressLine2: `Lat: ${coordinates.latitude.toFixed(
      6
    ...
    formattedAddress: `Location: ${coordinates.latitude.toFixed(
      6
 ...
 */
export const isWithinBangalore = async (
  coordinates: LocationCoordinates
): Promise<boolean> => {
  const { latitude, longitude } = coordinates;
  ...
  try {
    ...
    const isBangaloreByName = bangaloreNames.some(
      (name) => city.includes(name) || name.includes(city)
    );
    ...
    if (pincode && pincode.length === 6) {
      ...
      isBangaloreByPincode = BANGALORE_PINCODE_RANGES.some(
    ...
    if (distanceKm <= 40) {
      console.log(
        "✅ Location within 40km of Bangalore center:",
    ...
    const bangaloreBounds = {
      north: 13.2, // Includes Devanahalli area
      south: 12.7, // Includes Bannerghatta
      east: 77.9, // Includes Whitefield
      west: 77.3, // Includes Peenya/Rajajinagar
    };
